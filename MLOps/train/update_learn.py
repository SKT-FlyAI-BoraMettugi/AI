...
def update_learning():
    # 데이터 로드 (훈련 & 검증 데이터 분리)
    train_dataset, validation_dataset = load_db_data(test_size=0.2)

    # 모델 및 토크나이저 로드
    model, tokenizer = load_trained_model('Bllossom/llama-3.2-Korean-Bllossom-3B')
    ...
    # MinIO에 업로드
    upload_model_to_minio("Nolli/finetuned_model", "models")